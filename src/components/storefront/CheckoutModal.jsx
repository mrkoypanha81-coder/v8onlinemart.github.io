...
import { 
  ...
  Phone, User, Printer, ShoppingBag, Clock, Sparkles, AlertCircle, Home, Wallet, CreditCard,
  ZoomIn, Download, Copy, Check
} from 'lucide-react';
...
export const CheckoutModal = () => {
  ...
  const enabledMethods = ['khqr', 'wallet', 'card', 'cod'].filter(key => paymentSettings[key]?.enabled !== false);
  const defaultMethod = enabledMethods[0] || 'khqr';
  ...
  const [paymentMethod, setPaymentMethod] = useState(defaultMethod); // 'khqr' | 'wallet' | 'card' | 'cod'
  ...
  // HD Full Screen QR Zoom & Copy state
  const [isQrZoomed, setIsQrZoomed] = useState(false);
  const [isCopied, setIsCopied] = useState(false);

  const handleCopyMerchantId = () => {
    const idToCopy = paymentSettings.khqr?.merchantId || '012888999';
  ...
  const handleDownloadQrImage = () => {
    const qrSrc = paymentSettings.khqr?.qrImage;
    if (!qrSrc) return;
    const a = document.createElement('a');
    a.href = qrSrc;
    a.download = `ABA-KHQR-${paymentSettings.khqr?.merchantName || 'V8-Mart'}.png`;
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/75 backdrop-blur-sm flex items-center justify-center p-3 sm:p-6 animate-fade-in">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          {step === 2 && (
            <div className="space-y-5">
              {/* Payment Tabs (Dynamically Controlled by Admin) */}
              <div className={`grid gap-2 ${
                enabledMethods.length === 4 ? 'grid-cols-2 sm:grid-cols-4' :
                enabledMethods.length === 3 ? 'grid-cols-3' :
                enabledMethods.length === 2 ? 'grid-cols-2' : 'grid-cols-1'
              }`}>
                {paymentSettings.khqr?.enabled !== false && (
                  <button
                    onClick={() => setPaymentMethod('khqr')}
                    className={`p-3 rounded-2xl border flex flex-col items-center justify-center space-y-1.5 transition cursor-pointer ${
                      paymentMethod === 'khqr'
                        ? 'border-rose-600 bg-rose-50 text-rose-700 ring-2 ring-rose-500/20 font-bold shadow-xs'
                ...
                {paymentSettings.wallet?.enabled !== false && (
                  <button
                    onClick={() => setPaymentMethod('wallet')}
                    className={`p-3 rounded-2xl border flex flex-col items-center justify-center space-y-1.5 transition cursor-pointer ${
                      paymentMethod === 'wallet'
                        ? 'border-emerald-650 bg-emerald-50 text-emerald-800 ring-2 ring-emerald-500/20 font-bold shadow-xs'
                        : 'border-slate-200 bg-white text-slate-600 hover:bg-slate-50'
                    }`}
                  >
                ...
                {paymentSettings.card?.enabled !== false && (
                  <button
                    onClick={() => setPaymentMethod('card')}
                    className={`p-3 rounded-2xl border flex flex-col items-center justify-center space-y-1.5 transition cursor-pointer ${
                      paymentMethod === 'card'
                        ? 'border-blue-600 bg-blue-50 text-blue-800 ring-2 ring-blue-500/20 font-bold shadow-xs'
                        : 'border-slate-200 bg-white text-slate-600 hover:bg-slate-50'
                    }`}
                  >
                    <CreditCard className="w-5 h-5 text-blue-600" />
                    <span className="text-[11px]">{lang === 'km' ? 'ប័ណ្ណឥណទាន' : 'Credit Card'}</span>
                  </button>
                )}
                ...
                {paymentSettings.cod?.enabled !== false && (
                  <button
                    ...
                    className={`p-3 rounded-2xl border flex flex-col items-center justify-center space-y-1.5 transition cursor-pointer ${
                      paymentMethod === 'cod'
                        ? 'border-emerald-600 bg-emerald-50 text-emerald-700 ring-2 ring-emerald-500/20 font-bold shadow-xs'
                        : 'border-slate-200 bg-white text-slate-600 hover:bg-slate-50'
                    }`}
                  >
              ...
              {paymentMethod === 'khqr' && paymentSettings.khqr?.enabled !== false && (
                <div className="bg-gradient-to-b from-rose-500 to-rose-700 rounded-3xl p-5 text-white shadow-xl space-y-4 animate-fade-in">
                  ...
                  {/* High Definition KHQR Frame */}
                  <div className="bg-white rounded-3xl p-5 text-slate-800 text-center space-y-4 shadow-inner">
                    <div className="space-y-0.5">
                      <div className="text-sm font-black text-slate-900">
                        Scan to Pay {paymentSettings.khqr?.merchantName || 'V8 Online Mart'}
                      </div>
                    </div>
                    
                    {/* Extra Large Interactive QR Container */}
                    <div 
                      onClick={() => setIsQrZoomed(true)}
                      className="w-full max-w-sm sm:max-w-md min-h-[320px] sm:min-h-[400px] bg-white mx-auto border-3 border-dashed border-rose-400 rounded-3xl flex flex-col items-center justify-center p-3 relative shadow-md hover:border-rose-600 transition group cursor-pointer overflow-hidden"
                      title="Click to Zoom Full Screen HD QR"
                    >
                      {paymentSettings.khqr?.qrImage ? (
                        <img
                          src={paymentSettings.khqr.qrImage}
                          alt="Custom ABA KHQR"
                          className="w-full h-full max-h-[420px] object-contain rounded-2xl group-hover:scale-105 transition-transform duration-300"
                        />
                      ) : (
                        <div className="w-full flex flex-col items-center justify-center p-4">
                          <svg className="w-64 h-64 sm:w-72 sm:h-72" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
                          ...
                          <div className="absolute inset-0 flex items-center justify-center">
                            <div className="bg-white/95 px-3 py-1 rounded-lg border-2 border-rose-500 shadow-md text-xs font-black text-rose-600">
                              V8 KHQR
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="absolute bottom-2 right-2 bg-slate-900/80 text-white text-[10px] font-bold px-2.5 py-1 rounded-full backdrop-blur-xs flex items-center space-x-1 opacity-90 group-hover:opacity-100 transition">
                        <ZoomIn className="w-3.5 h-3.5 text-rose-400" />
                        <span>{lang === 'km' ? '🔍 ចុចមើល QR ធំបំផុត (HD Zoom)' : '🔍 Click to Zoom HD'}</span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-center gap-2 pt-1 flex-wrap">
                      <button
                        type="button"
                        onClick={() => setIsQrZoomed(true)}
                        className="px-3.5 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-800 rounded-xl text-xs font-bold transition flex items-center space-x-1.5 cursor-pointer"
                      >
                        <ZoomIn className="w-3.5 h-3.5 text-rose-600" />
                        <span>{lang === 'km' ? '🔍 ពង្រីកធំ' : 'Zoom HD'}</span>
                      </button>
                      ...
                      {paymentSettings.khqr?.qrImage && (
                        ...
                        >
                          ...
                          <span>{lang === 'km' ? '📥 ទាញយករូប' : 'Download Image'}</span>
                        </button>
                      )}
                    </div>

                    <div>
                      <div className="text-2xl font-black text-slate-900 font-mono tracking-tight">{formatPrice(cartTotal)}</div>
                      <div className="text-xs text-slate-500 font-bold mt-0.5">≈ {formatDualPrice(cartTotal).khr}</div>
                    </div>
                  </div>
      ...
      {/* ============================================================== */}
      {/* FULL SCREEN ULTRA-HD QR CODE ZOOM MODAL                        */}
      {/* ============================================================== */}
      {isQrZoomed && (
        <div className="fixed inset-0 z-[100] bg-slate-950/90 backdrop-blur-md flex items-center justify-center p-4 animate-fade-in font-sans">
          <div className="bg-white dark:bg-slate-900 w-full max-w-xl rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-4 relative flex flex-col items-center text-center">
            ...
            <div className="space-y-1">
              <span className="px-3 py-1 bg-rose-100 dark:bg-rose-950 text-rose-600 dark:text-rose-300 text-xs font-black rounded-full border border-rose-300">
                🔍 ABA KHQR High-Definition Scan View
              </span>
              <h3 className="font-black text-lg text-slate-900 dark:text-white">
                {paymentSettings.khqr?.merchantName || 'V8 Online Mart'}
              </h3>
            ...
            {/* Extra Large Image Display */}
            <div className="w-full max-w-md h-[460px] bg-white rounded-3xl p-3 border-2 border-rose-500 shadow-xl flex items-center justify-center overflow-hidden">
              {paymentSettings.khqr?.qrImage ? (
                <img
                  src={paymentSettings.khqr.qrImage}
                  alt="HD ABA KHQR Standee"
                  className="w-full h-full object-contain rounded-2xl"
                />
              ) : (
                <div className="w-full flex items-center justify-center">
                  <QrCode className="w-64 h-64 text-rose-600" />
                </div>
              )}
            </div>
            ...
            <div className="flex items-center space-x-3 pt-2">
              {paymentSettings.khqr?.qrImage && (
                <button
                  onClick={handleDownloadQrImage}
                  className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-black text-xs rounded-2xl shadow-md transition flex items-center space-x-2 cursor-pointer"
                >
                  <Download className="w-4 h-4" />
                  <span>{lang === 'km' ? '📥 ទាញយករូបភាព QR នេះ' : 'Download HD QR'}</span>
                </button>
              )}

              <button
                onClick={() => setIsQrZoomed(false)}
                className="px-6 py-2.5 bg-slate-200 dark:bg-slate-800 text-slate-800 dark:text-white font-bold text-xs rounded-2xl cursor-pointer"
              >
                {lang === 'km' ? 'បិទផ្ទាំងពង្រីក' : 'Close Zoom'}
              </button>
            </div>
          </div>
        </div>
